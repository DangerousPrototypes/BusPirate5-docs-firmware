...
|-----|-----------------|
| 0   | Macro menu                                                      |
| 1   | 1-Wire ROM search. Find all connected device IDs. |
| 2   | Read temperature from a single 18B20 sensor.|

### 1-Wire ROM search macro
...
Macro ```(1)``` performs a 1-Wire ROM search to detect the ID of every connected 1-Wire device. They type of device is shown if the family ID is known.

### Read 18B20 temperature
...
Macro ```(2)``` reads the temperature from a single 18B20 temperature sensor. The macro uses the skip ROM command, so it will only work when the DS18B20 is the only device connected.
