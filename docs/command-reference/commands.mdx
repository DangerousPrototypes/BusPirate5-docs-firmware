...
Flash Card: <span className="bp-float"> 64.06GB</span> (EXFAT File System)<br/>
<br/>
...
If a flash card is inserted your terminal mode choice will be saved and automatically loaded the next time the Bus Pirate is connected. Just press ```enter``` to start the command prompt.
...
:::tip
If you're stuck in a terminal mode that's not working, remove the flash card and restart the Bus Pirate. You can also navigate to the Bus Pirate USB disk, delete the bpconfig.bp file and restart the Bus Pirate. You will be prompted to choose VT100 or ASCII mode after restarting. 
:::
...
If a flash card is inserted many options will be saved and can be reloaded later.

## Disk Commands
Several Linux-like disk commands can be used to navigate a flash card inserted in the Bus Pirate.
...
List the contents of the current directory on the flash card. Type ```ls``` followed by ```enter```. ``ls`` followed by a directory name lists the contents of that directory.
...
Make directory in the current location on the flash card. Type ```mkdir``` followed by a directory name and then ```enter```.
...
Bus Pirate 5 can program and dump EEPROM, flash and other memory chips directly to the flash card! No need for external software on your computer. No need to install toolchains and compile scripts. What you need, where you need it. Bus Pirate 5 is here! --EOM<br/>
<br/>
<span className="bp-prompt">HiZ></span> <br/>
</BrowserWindow>
...
S/N: <span className="bp-float">2509449B952069E4</span><br/>
https://DangerousPrototypes.com/<br/>
Flash card: <span className="bp-float"> 64.06GB</span> (FAT32 File System)<br/>
<br/>
<span className="bp-info">Configuration file:</span> Loaded<br/>
<span className="bp-info">Available modes:</span> HiZ UART I2C SPI LED DUMMY1<br/>
<span className="bp-info">Active mode:</span> HiZ ()=()<br/>
...
Type ```c``` followed by ```enter``` to show the configuration menu. If a flash card is inserted settings will be saved and loaded automatically when the Bus Pirate restarts.
