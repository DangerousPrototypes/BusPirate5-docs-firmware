...
## Disk Commands
Several Linux-like disk commands can be used to navigate an SD card inserted in the Bus Pirate.
...
List the contents of the current directory on the SD card. Type ```ls``` followed by ```enter```. ``ls`` followed by a directory name lists the contents of that directory.
...
Make directory in the current location on the SD card. Type ```mkdir``` followed by a directory name and then ```enter```.
...
Bus Pirate 5 can program and dump EEPROM, flash and other memory chips directly to the SD card! No need for external software on your computer. No need to install toolchains and compile scripts. What you need, where you need it. Bus Pirate 5 is here! --EOM<br/>
<br/>
<span className="bp-prompt">HiZ></span> <br/>
</BrowserWindow>
