...
- Insert a TF flash card. If **a formatted and mountable TF flash card is not inserted the TF flash card test will fail**. That doesn't mean it's broken, it just means the test couldn't be completed.

:::warning
Disconnect any devices before performing the self-test.
:::
:::caution
The test will work without a TF flash card, but the TF flash card section of the self-test will fail.
:::
...
- **TF FLASH CARD** - Try to mount a TF flash card. **If no TF flash card is inserted this test will fail**. Tests the RP2040 connections and TF flash card socket.
