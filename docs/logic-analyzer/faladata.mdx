...
:::caution
There is currently no software support for FALA. The protocol is a work in progress. The GusmanB logic analyzer software has come a long way, and it supports asynchronous capture notifications. The current protocol may change to use the GusmanB protocol.
:::
