...
sidebar_label: 'AVR Arduino Leonardo'
---
...
This makes it possible to program the ATmega32U4 chip found on the Arduino Leonardo through the ICSP header. 
...
- Download AVRDUDE from: https://github.com/avrdudes/avrdude
- Download AVRDUDESS from: https://github.com/ZakKemble/AVRDUDESS

## Connections

![](./img/leonardo-icsp-pinout.png)

The Bus Pirate can be connected to the ICSP header using the following connections:

|Bus Pirate|Arduino Leonardo|Description|
|-|-|-|
|MISO|ICSP-1|Master In Slave Out|
|5V|ICSP-2|Power Supply|
|CLK|ICSP-3|Clock|
...
:::caution 
Arduino Leonardo runs at 5V. Make sure the Bus Pirate is set to 5V mode.
:::

## Setup

Enter to binmode and select "Legacy Binary Mode for Flashrom and AVRdude (EXPERIMENTAL)"
...
When entering this mode, it asks for the power supply voltage. Select 5V 
...
150 ma is a good value for the current limit. 

## Dumping flash memory

It's necessary to use the binmode COM port (which is not the same as the one used to enter binmode).

To dump the flash memory of the ATmega32U4, use the following command:
...
:::tip
For Linux, the COM port is usually a `/dev/ttyUSB`.
:::

This command reads the flash memory of the ATmega32U4 and saves it to a file named `dump.bin`.
