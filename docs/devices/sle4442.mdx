...
Connect the Bus Pirate to the smart card as shown in the table above. It's possible to gently solder wires into each pad of the chip, but a KF-011C (or similar) smart card socket is useful if you don't want to destroy the card.

:::tip
A smart card and SIM card adapter is available for Bus Pirate 5 with the correct connections already set. The adapter accepts most ISO 7816-3 smart cards and mini/micro/nano SIM cards. 
:::
